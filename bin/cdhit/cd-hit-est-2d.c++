...
int calc_ann_list(int len, char *seqi,
  ...
  if      ( NAA == 2)
    ...
    for (j=0; j<aan_no; j++)
      aan_list[j] =
        seqi[j]*NAA5+
        seqi[j+1]*NAA4+seqi[j+2]*NAA3+seqi[j+3]*NAA2+seqi[j+4]*NAA1+seqi[j+5];
  else if ( NAA == 7)
    for (j=0; j<aan_no; j++)
      aan_list[j] =
        seqi[j]*NAA6+seqi[j+1]*NAA5+
        seqi[j+2]*NAA4+seqi[j+3]*NAA3+seqi[j+4]*NAA2+seqi[j+5]*NAA1+seqi[j+6];
  else if ( NAA == 8)
    for (j=0; j<aan_no; j++)
      aan_list[j] =
        seqi[j]*NAA7+seqi[j+1]*NAA6+seqi[j+2]*NAA5+
        seqi[j+3]*NAA4+seqi[j+4]*NAA3+seqi[j+5]*NAA2+seqi[j+6]*NAA1+seqi[j+7];
  else if ( NAA == 9)
    for (j=0; j<aan_no; j++)
      aan_list[j] =
        seqi[j]*NAA8+seqi[j+1]*NAA7+seqi[j+2]*NAA6+seqi[j+3]*NAA5+
        seqi[j+4]*NAA4+seqi[j+5]*NAA3+seqi[j+6]*NAA2+seqi[j+7]*NAA1+seqi[j+8];
  else if ( NAA == 10)
    for (j=0; j<aan_no; j++)
      aan_list[j] =
        seqi[j]*NAA9+seqi[j+1]*NAA8+seqi[j+2]*NAA7+seqi[j+3]*NAA6+
        seqi[j+4]*NAA5+
        seqi[j+5]*NAA4+seqi[j+6]*NAA3+seqi[j+7]*NAA2+seqi[j+8]*NAA1+seqi[j+9];
