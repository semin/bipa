...
#
#  def filtered_children(redundancy, resolution)
#    children.send("rep#{redundancy}").send("res#{resolution}")
#  end
#  memoize :filtered_children
#
#  def all_filtered_children(redundancy, resolution)
    ...
    %w(mean stddev).each do |property|
      class_eval <<-END
        def #{property}_#{na}_interface_asa
          #{na}_interfaces.map(&:asa).to_stats_array.#{property}
        end
      ...
      AminoAcids::Residues::STANDARD.map(&:downcase).each do |aa|
        class_eval <<-END
          def #{property}_#{na}_interface_singlet_propensity_of_#{aa}
            #{na}_interfaces.map(&:singlet_propensity_of_#{aa}).to_stats_array.#{property}
          end
      ...
      Dssp::SSES.map(&:downcase).each do |sse|
        class_eval <<-END
          def #{property}_#{na}_interface_sse_propensity_of_#{sse}
            #{na}_interfaces(redundancy, resolution = nil).map(&:sse_propensity_of_#{sse}).to_stats_array.#{property}
          end
